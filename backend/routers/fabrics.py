...
def update_fabric(fabric_id: str, updates: FabricUpdate, decoded_token: dict = Depends(verify_token)):
    uid = decoded_token["uid"]
    doc = db.collection("fabrics").document(fabric_id).get()
    if not doc.exists:
        raise HTTPException(status_code=404, detail="Fabric not found")
    if doc.to_dict().get("supplier_id") != uid:
        raise HTTPException(status_code=403, detail="You can only update your own fabric listings")
