...
def process_ai_chat(prompt: str, user_measurements: dict, fabrics: list):
    ...
    if needs_size_match:
        if user_measurements:
            ...
            # Simple assumption:
            if "shirt" in prompt_lower or "top" in prompt_lower or "dress" in prompt_lower:
                required_meters = round(chest * 0.025 + 0.5, 1)  # ex: 100cm chest -> 3.0 meters
            elif "pant" in prompt_lower or "denim" in prompt_lower or "trouser" in prompt_lower:
                required_meters = round(waist * 0.025 + 0.8, 1)  # ex: 80cm waist -> 2.8 meters
            else:
