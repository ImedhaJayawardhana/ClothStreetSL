...
def process_ai_chat(prompt: str, user_measurements: dict, fabrics: list, cart_items: list = None, tailors: list = None, designers: list = None):
    ...
    # Analyze intent
    needs_size_match = "fits me" in prompt_lower or "my size" in prompt_lower or "calculate" in prompt_lower
    needs_provider_match = ("matching" in prompt_lower or "perfect" in prompt_lower or "find" in prompt_lower) and \
                          ("tailor" in prompt_lower or "designer" in prompt_lower)
    ...
    # 1. Colors & Materials
    color_keywords = ["white", "black", "blue", "red", "green", "yellow", "pink", "purple", "brown", "grey", "gray"]
    type_keywords = ["cotton", "silk", "linen", "polyester", "denim", "chiffon", "wool", "rayon", "velvet"]
    ...
    for f in fabrics:
        ...
        if found_colors and not any(c in f_name_lower or c in f_color_lower for c in found_colors):
            continue
        if found_types and not any(t in f_name_lower or t in f_type_lower for t in found_types):
            continue
    ...
    if needs_size_match:
        if user_measurements and ("chest" in user_measurements or "waist" in user_measurements):
...
def chat_with_ai(request: ChatRequest):
    ...
    fabrics = [doc.to_dict() | {"id": doc.id} for doc in fabrics_ref if not doc.to_dict().get("hidden")]
    ...
    designers = [doc.to_dict() | {"id": doc.id, "type": "designer"} for doc in designers_ref]

    # 3. Process the AI intent
    response = process_ai_chat(request.prompt, measurements, fabrics, cart_items, tailors, designers)
