...
def get_quotation(quotation_id: str, decoded_token: dict = Depends(verify_token)):
    ...
    if uid != data.get("customer_id") and uid != data.get("tailor_id"):
        raise HTTPException(status_code=403, detail="You are not involved in this quotation")
    data["id"] = doc.id
    return data


@router.patch("/{quotation_id}/status")
def update_quotation_status(quotation_id: str, status_update: QuotationStatusUpdate, decoded_token: dict = Depends(verify_token)):
    uid = decoded_token["uid"]
    doc = db.collection("quotations").document(quotation_id).get()
    if not doc.exists:
        raise HTTPException(status_code=404, detail="Quotation not found")
    if uid != doc.to_dict().get("tailor_id"):
        raise HTTPException(status_code=403, detail="Only the tailor can update quotation status")
    db.collection("quotations").document(quotation_id).update({"status": status_update.status})
