...
def create_quotation(
    quotation: Quotation, decoded_token: dict = Depends(verify_token)
):
...
def get_quotation(
    quotation_id: str, decoded_token: dict = Depends(verify_token)
):
    uid = decoded_token["uid"]
...
def update_quotation(
    ...
    if update_data.get("status", "").lower() == "accepted":
        ...
        items.append({
            ...
            "quantity": 1,
            "unit": "service",
            "unitPrice": data.get("laborCharge", 0) + data.get("additionalCharges", 0)
        })

        order_data = {
            "id": "", # Will be replaced by doc_ref.id implicitly in firestore, but keeping format clean
            "customerId": data.get("customerId", ""),
            "customer_id": data.get("customerId", ""), # legacy field to support existing UI
...
def delete_quotation(
    ...
    if data.get("status") not in ["pending", "rejected", "declined"]:
        raise HTTPException(
            status_code=400, detail="Cannot delete a quotation that is in progress or completed"
        )
