...
def cleanup_cart_item(customer_id: str, provider_id: str, decoded_token: dict = Depends(verify_token)):
    ...
    try:
        ...
        for d in docs:
            item = d.to_dict()
            if item.get("providerId") == provider_id or item.get("serviceType") == provider_id:
