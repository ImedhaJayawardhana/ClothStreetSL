...
def get_shopify_images(url):
    ...
    try:
        ...
        matches1 = re.findall(
            r"(//cdn\.shopify\.com/s/files/[^\s\"\'\?]+\.(?:jpg|png|webp))", r.text
        )
        # Match custom domain shopify cdn links
        domain = url.split("//")[1].split("/")[0]
        matches2 = re.findall(
            rf"(//{domain}/cdn/shop/files/[^\s\"\'\?]+\.(?:jpg|png|webp))", r.text
        )
...
def get_nolimit_images():
    ...
    try:
        ...
        for img in soup.find_all("img"):
            src = img.get("src") or ""
            if "product" in src.lower() and (
                "jpg" in src.lower() or "png" in src.lower() or "webp" in src.lower()
            ):
                if src.startswith("//"):
                    src = "https:" + src
                elif src.startswith("/"):
...
def seed_db():
    ...
    moose_imgs = get_shopify_images(
        "https://mooseclothingcompany.com/collections/mens-t-shirts"
    )
    mens_pool = nolimit_imgs + moose_imgs

    # Women's
    mimosa_imgs = get_shopify_images("https://mimosaforever.com/collections/skirts")
    zigzag_imgs = get_shopify_images(
        "https://www.zigzag.lk/collections/womens-clothing"
    )
    ...
    for doc in docs_all:
        ...
        if (
            "seller_store_" in sid
            or "seller_nolimit" in sid
            or "seller_mimosa" in sid
            or "seeder" in sid
        ):
