...
export default function FindTailorDesigner() {
    ...
    return (
        <div className="min-h-screen relative pb-24">
            {/* ───────────── HERO BANNER ───────────── */}
            <section className="relative overflow-hidden bg-gradient-to-br from-amber-950 via-amber-900 to-amber-950 py-16 px-4">
                ...
                <div className="max-w-5xl mx-auto relative z-10">
                    <button
                        onClick={handleBack}
                        className="absolute -top-4 left-0 inline-flex items-center gap-2 text-sm font-semibold hover:text-white text-amber-200 transition-colors cursor-pointer"
                    >
                    ...
                    <div className="text-center pt-8">
                        <span className="inline-flex items-center gap-1.5 text-xs font-semibold tracking-widest text-amber-200 uppercase bg-amber-500/15 border border-amber-400/25 rounded-full px-4 py-1.5 mb-5">
                        ...
                        <p className="text-base text-amber-200/70 max-w-lg mx-auto">
                            {pageSubtitle}
                        </p>
                    </div>
                </div>
                ...
                <div className="rounded-2xl shadow-xl shadow-gray-200/60 border bg-white p-4 sm:p-5">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full border border-gray-200 focus:border-amber-400 focus:ring-2 focus:ring-amber-100 rounded-xl pl-12 pr-10 py-3 text-sm placeholder-gray-400 outline-none transition-all duration-200"
                        />
                        {searchQuery && (
                            <button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                        {[1, 2, 3, 4, 5, 6].map((i) => (
                            <div key={i} className="rounded-2xl border border-gray-100 bg-white overflow-hidden shadow-sm animate-pulse">
                                <div className="h-2 bg-gradient-to-r from-amber-100 to-amber-100" />
                            ...
                            return (
                                <div
                                    ...
                                    className={`rounded-2xl bg-white border-2 overflow-hidden transition-all duration-200 cursor-pointer flex flex-col relative
                                        ${isSelected
                                            ? "border-amber-500 shadow-lg shadow-amber-100 transform -translate-y-1"
                                ...
                                >
                                    ...
                                    <div className={`absolute top-4 right-4 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors z-10
                                        ${isSelected ? "bg-amber-500 border-amber-500 text-white" : "border-gray-300 bg-white/50"}`}>
                                        {isSelected && (
                                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    {/* Top accent bar */}
                                    <div className={`h-1.5 w-full ${isTailor ? "bg-gradient-to-r from-amber-500 to-amber-500" : "bg-gradient-to-r from-rose-500 to-pink-500"}`} />
                                    ...
                                    <div className="p-6 flex flex-col flex-grow">
                                        ...
                                        <div className="flex items-start gap-3.5 mb-4 pr-8">
                                            ...
                                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg shrink-0 text-white shadow-inner
                                                ${isTailor ? "bg-gradient-to-br from-amber-500 to-amber-600" : "bg-gradient-to-br from-rose-500 to-pink-600"}`}>
                                                {provider.name?.charAt(0) || "?"}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <h3 className={`text-base font-bold truncate transition-colors ${isSelected ? "text-amber-700" : "text-gray-900"}`}>
                                                    {provider.name}
                                                </h3>
                                        ...
                                        <div className="flex flex-wrap gap-2 mb-4">
                                            <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider
                                                ${isTailor ? "bg-amber-50 text-amber-700 border border-amber-100" : "bg-rose-50 text-rose-700 border border-rose-100"}`}>
                                    ...
                                    <div className={`h-1 w-full transition-colors ${isSelected ? "bg-amber-500" : "bg-transparent"}`} />
                                </div>
                            );
                        })}
                    </div>
                ...
                <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-4 w-full sm:w-auto">
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg text-white shadow-inner
                            ${selectedProviderDetails?.providerType === "tailor" ? "bg-gradient-to-br from-amber-500 to-amber-600" : "bg-gradient-to-br from-rose-500 to-pink-600"}`}>
                    ...
                    <button
                        ...
                        className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-3.5 bg-gradient-to-r from-amber-600 to-amber-600 hover:from-amber-700 hover:to-amber-700 text-white text-base font-bold rounded-xl transition-all shadow-md shadow-amber-200 hover:shadow-lg hover:shadow-amber-300 transform hover:-translate-y-0.5"
                    >
