...
const STATUS_MAP = {
  ...
  quoted: { label: "Quoted", bg: "bg-amber-100", text: "text-amber-700", dot: "bg-amber-500" },
  ...
  completed: { label: "Completed", bg: "bg-amber-100", text: "text-amber-700", dot: "bg-amber-700" },
};
    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 space-y-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { label: "Incoming Requests", value: stats.total, icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeWidth="2.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>, bg: "bg-amber-50", accent: "text-amber-600" },
            ...
            <div key={stat.label} className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 hover:shadow-md hover:border-amber-100 transition-all group">
          ...
          <div className="space-y-6">
            {filteredQuotations.map((q) => {
              ...
              return (
                <div
                  key={q.id}
                  className="bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:border-amber-200 hover:-translate-y-1 transition-all cursor-pointer group relative overflow-hidden"
                  onClick={() => navigate(`/quotation-response/${q.id}`, { state: { quotation: q } })}
                >
                  <div className="p-8">
                    <div className="flex items-start gap-6">
                      {/* Brand Avatar */}
                      <div className="w-16 h-16 rounded-2xl bg-slate-900 flex items-center justify-center font-black text-2xl text-white shrink-0 shadow-xl shadow-slate-200 group-hover:bg-amber-600 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="text-lg font-black text-slate-900 truncate group-hover:text-amber-600 transition-colors">
                        ...
                        <div className="flex flex-wrap items-center gap-6 text-[10px] font-black uppercase tracking-widest text-slate-400">
                          <span className="flex items-center gap-2">
                            <svg className="w-4 h-4 text-amber-500" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24">
                          ...
                          {q.designImages?.length > 0 && (
                            <span className="flex items-center gap-2 bg-amber-50 px-2 py-1 rounded-md text-amber-600">
                  ...
                  {q.status === "completed" && (
                    <div className="px-8 pb-6 pt-0">
                      <div className="w-full py-3 bg-amber-50 text-amber-700 text-xs font-black uppercase tracking-widest rounded-2xl flex items-center justify-center gap-2 border border-amber-100">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24">
