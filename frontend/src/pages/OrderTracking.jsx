  ...
  if (error || !order) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <button onClick={() => navigate("/orders")} className="px-6 py-2.5 rounded-xl bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold transition-all">
          Back to My Orders
        </button>
      </div>
    );
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          ...
          <div className="hidden sm:block">
            <div className="relative mb-2">
              ...
              <div className="absolute top-5 left-6 h-1 bg-gradient-to-r from-violet-500 to-purple-500 rounded-full transition-all duration-700"
              ...
              {legs.map((leg, idx) => {
                ...
                return (
                  <div key={leg.key} className="flex flex-col items-center" style={{ width: `${100 / legs.length}%` }}>
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg border-2 transition-all z-10 ${isCompleted ? "bg-violet-600 border-violet-600 text-white shadow-md shadow-violet-200" : isCurrent ? "bg-white border-violet-500 shadow-lg shadow-violet-100 ring-4 ring-violet-100" : "bg-white border-slate-200"}`}>
                      {isCompleted ? (
                    ...
                    <p className={`text-[11px] font-semibold text-center mt-2 leading-tight max-w-[80px] ${isCompleted ? "text-violet-600" : isCurrent ? "text-slate-800" : "text-slate-400"}`}>
                      {leg.label}
                    </p>
                  </div>
                );
              ...
              return (
                <div key={leg.key} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm border-2 shrink-0 z-10 ${isCompleted ? "bg-violet-600 border-violet-600 text-white" : isCurrent ? "bg-white border-violet-500 ring-4 ring-violet-100" : "bg-white border-slate-200"}`}>
                      {isCompleted ? (
                    ...
                    {!isLast && <div className={`w-0.5 flex-1 min-h-[24px] ${isCompleted ? "bg-violet-400" : "bg-slate-100"}`} />}
                  </div>
                  <div className="pb-4">
                    <p className={`text-sm font-semibold ${isCompleted ? "text-violet-600" : isCurrent ? "text-slate-800" : "text-slate-400"}`}>{leg.label}</p>
                    {isCurrent && <span className="inline-flex items-center mt-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-violet-100 text-violet-700">Current</span>}
                  </div>
                </div>
              );
            })}
              ...
              {displayTimeline.map((event, idx) => {
                ...
                return (
                  <div key={idx} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`w-3 h-3 rounded-full shrink-0 z-10 ${isLatest ? "bg-violet-600 ring-4 ring-violet-100" : "bg-slate-300"}`} />
        ...
        {quotation?.designDeliverables?.length > 0 && (
          <div className="bg-white rounded-2xl border-2 border-emerald-100 shadow-sm p-6">
            ...
            {quotation.designDeliveryMessage && (
              <p className="text-sm text-slate-600 bg-indigo-50 rounded-xl p-4 mb-4 whitespace-pre-wrap border border-indigo-100">
                <span className="font-bold text-indigo-700 block mb-1">Message from Designer:</span>
                {quotation.designDeliveryMessage}
              </p>
            )}
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {quotation.designDeliverables.map((url, idx) => (
                <a key={idx} href={url} target="_blank" rel="noopener noreferrer"
                  className="group rounded-xl border-2 border-slate-100 hover:border-violet-200 p-3 flex flex-col items-center gap-2 transition-all hover:shadow-md">
                  ...
                  <span className="text-xs font-semibold text-violet-600 group-hover:text-violet-700">Download File {idx + 1}</span>
                </a>
              ))}
            </div>
          </div>
